...
export const checkUpdates = async () => {
	...
	// Check if appSpec.id exists in apps and if it needs to be removed
	for (const app of apps) {
		if (!appsSpecs.find((a) => a.id === app.id)) {
	...
	console.log("Done updating apps");
}
...
async function generateAppFields(
	...
	if (
		...
		return {
			...
			name: manifest?.name || manifest?.short_name || appCurrentData?.name || "",
