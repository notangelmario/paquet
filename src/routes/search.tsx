...
export default async function Search(req: Request) {
	...
	const apps = await dbGet<App[]>(`select id, name, icon, categories from apps where name like '%${query}%';`);

	const categories = searchCategory(query);

	let moreApps: App[] = [];
	if (apps && apps.length < 10) {
		const data = await dbGet<App[]>(`select id, name, icon, categories from apps where id not in (${apps.map((app) => app.id).join(",")}) order by random() limit 5;`);
